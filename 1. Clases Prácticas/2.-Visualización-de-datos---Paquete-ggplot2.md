...
### 1.2 Modificaciones adicionales de geom\_bar()

Podemos generar gráficos de barras con dos variables cualitativas. Ello
para visualizar el cambio de los valores analizados en función de estas
2 variables. POr ejemplo:

``` r
starwars %>% 
  drop_na(sex) %>% 
  filter(species %in% c("Human", "Droid", "Gungan", "Kaminoan", "Mirialan", "Wookiee")) %>% 
  ggplot()+
  geom_bar(mapping = aes(x=sex, fill = species),color = "black")
```

![](2.-Visualización-de-datos---Paquete-ggplot2_files/figure-gfm/unnamed-chunk-6-1.png)<!-- -->

En el ejemplo anterior hemos filtrado algunas de las especies con mayor
frecuencia en la base de datos. Luego hemos utilizado el argumento fill=
(dentro de la función geom\_bar()) para añadir la nueva división en
función a la variable species. El gráfico tiene tanto la clasificación
por sex + species, brindando mayor información en un solo gráfico (lo
cual variará dependiendo de la exploración de la base de datos o los
objetivos del investigador).

Está no es la única manera en que podemos gráficar, probemos otros
...
Opcionalmente, podemos usar una versión muy parecida a la anterior, pero
...
De esta manera se preservan las barras separadas como unicas (esto lo
logramos incorporando una función al argumento position =, el cual es la
función position\_dodge() y dentro de la misma, el argumento “single”).
...
![](2.-Visualización-de-datos---Paquete-ggplot2_files/figure-gfm/unnamed-chunk-12-1.png)<!-- -->
...
![](2.-Visualización-de-datos---Paquete-ggplot2_files/figure-gfm/unnamed-chunk-13-1.png)<!-- -->
...
![](2.-Visualización-de-datos---Paquete-ggplot2_files/figure-gfm/unnamed-chunk-14-1.png)<!-- -->
...
## ¿Como eliminar la notación científica?

En R, muchas veces se utilizará la constante de euler(e), como
representación de un valor númerico. Esto ocurre por que R esta
programado con representación númerica en notación científica. Si
queremos evitar ello, podemos incorporar ciertos cambios en el código,
por ejemplo, tenemos la siguiente base de datos:

``` r
set.seed(123)                             # Definimos una regla para impedir la aleatoridad
data <- data.frame(x = runif(1000,        # Creamos una base de datos, con números grandes
                             100000000,
                             200000000),
                   y = rnorm(1000))
```

Gráfiquemos ahora en ggplot las dos variables:
...
![](2.-Visualización-de-datos---Paquete-ggplot2_files/figure-gfm/unnamed-chunk-16-1.png)<!-- -->

La notación cientifica esta dominada por la configuración del lenguaje,
para cambiarlo, podemos correr el comando options(), y como argumento
aumentar scipen =. Si scipen tiene el valor de “0”, entonces sera
notación científica. Dando un valor mayor a cero o grande (dependiendo
del número de cifras en nuestra variable), realizará la magia de
eliminar la notación científica. De lo dicho:
...
![](2.-Visualización-de-datos---Paquete-ggplot2_files/figure-gfm/unnamed-chunk-17-1.png)<!-- -->

Se puede ver ahora todo el número. Si bien esta función nos ayuda, quizá
puede ser mejor cambiar las unidades de nuestra variable, si esta en
kilogramos por ejemplo, transformarlo a toneladas, etc.

Base de datos pingüinos: library(palmerpenguins)
