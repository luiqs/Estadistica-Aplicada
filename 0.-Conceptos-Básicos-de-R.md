...
Antes de comenzar a escribir codigos y interactuar con R, debemos de
crear un nuevo proyecto. Para ello, iremos a la ventada de Archivo
(File) -&gt; Nuevo proyecto (New Project).

![](NewProject.PNG) ![](NewProject2.PNG) ![](NewProject3.PNG)
...
figura (quiza con otros colores de fondo y letras):
...
programacion R, utilizando Rstudio, escribimos todo nuestro codigo en un
...
Rstudio para escrir nuestros codigos y tiene sus caracateristicas. Sin
embargo, en los ultimos años, una actualización de este “script”
...
Repasemos algunos concepto basicos de un script. Para su creación vamos
a la opcion Archivo (File) del programa y colocamos nuevo archivo -&gt;
Script.

![](Script.PNG)

Esto abrira un script en blanco sin nombre y sin guardarse
auutomaticamente, tendremos que seleccionarlo e ir a archivo -&gt;
Guardar como (colocamos el nombre de nuestro archivo y la terminación
.R). Cuando guardemos, este nuevo archio aparecerá en nuestra pesataña
de Archivos donde antes solo estaba el proyecto.

![](SinNombre.PNG) ![](GuardaComo1.PNG) ![](GuardaComo2.PNG)
![](GuardaComo3.PNG)

Una vez creado el script y guardado, podemos comenzar a codificar y
obtener resultados fantasticos con nuestras bases de datos.

Por otro lado, para crear un archivo Rmarkdown, los pasos son muy
parecidos. Vamos a Archivo -&gt; Nuevo archivo -&gt;

![](Markdown1.PNG) ![](Markdown2.PNG) ![](Markdown3.PNG)
...
Nota: el tipo de archico ya no es un archivo “.R”, es en cambio un tipo
de archivo “.Rmd”.
...
de este tipo de formato se aprenderá a final del curso de estadistica
aplicada para la ingenieria).

Una vez tengamos un script abierto, guardado y en blanco, podemos
comenzar con los primeros pasos en R, la instalacion de paquetes y los
...
Rstudio). Por ejemplo, una función para entender esta interaccion es el
...
Cuando escriban esto en su script y le den al boton de Correr o “Run” en
la parte superior del Script, despues de unos segundos de carga, lo
veran aparecer como resultado en la consola. Podemos escribir mucho
codigo en nuestros scripts pero si no damos click en “Run”, este codigo
nunca se ejecutará y nunca veremos el resultado. En la figura se puede
ver el boton de Run resaltado.

![](Run.PNG) ![](Run2.PNG)

Como se puede ver, para correr un “pedazo” de codigo especifico tenemos
dos opciones, sobrear todo lo que queremos “Correr” o colocar el mouse
en alguna parte de la linea de codigo que queramos correr (que es quiza
lo mas práctico, solo aseguremosnos que el curso del texto esta en
alguna parte de la linea de codigo). Para trabajar de manera práctica,
sin depender del mouse, podemos utilizar el comando rapido para “Correr”
codigos, este es usando las teclas *Ctrl + Enter*. De esta manera no
tendremos que recurrrir a apretar siempre el boton de “Run”.
...
ofrece informacion general, como el numero de filas (individuos) y
columnas (variables).

![](BaseDatos.PNG)

En la figura observamos por ejemplo que tiene 234 filas y 11 columnas
(rojo). Tambien podemos observar que cada una de las variables tiene una
etiqueta de tres letras, por ejemplo <chr>, <dbl>, <int>, etc. Las
cuales hemos visto en clase y nos da la informacion de que tipo de
...
Tambien podemos observar al comienzo de la tabla que se trata de un
...
formatos se siguen utilizando el dia de hoy. Si tenemos un dataframe y
...
tenerlas a la mano. Asi como cuando preparamos la mochila para un viaje
e incorporamos objeto por objeto, tambien lo hacemos en nuestra ventana
...
correr una tecnica estadistica o realizar un grafico en Rstudio.

Ya hemos visto como cargar bases de datos a nuestro ambiente con la
funcion(data), pero esto solo funciona cuando nosotros cargamos bases de
...
de asignar una variable. Antes de hacer este ejercicio, es recontra
...
utilizaremos los simbolos “&lt;-”. Entonces seria de la siguiente
manera:
...
Una vez corramos este codigo (no aparecera ningun resultado en la
consola), veran que despues de uno segundo, aparecerá en nuestra ventana
...
susceptible a los caracteres y diferencia entre mayusculas y minusculas,
espacios, etc. Por ejemplo si ahora queremos asignarla a una nueva
variable “x” (**miniscula**) el valore de 5, podremos hacerlo de la
siguiente manera:
...
Podremos ver que efectivamente hay un nuevo valor llamado x (minuscula)
y que se le asigno el valore de 5.

![](Variable2.PNG)

¿Qué pasa si le asignamos un nuevo valor a la variable X o x? Lo que
ocurrira es que el nuevo valor que le asignamos reemplazará al anterior
y esto lo podremos ver en la ventana de “Ambiente” una vez corramos el
siguiente codigo (donde asignamos el valore de 6 a X:
...
de gran numero. Podemos asignar a una variable un grafico, un modelo,
...
Veremos que en nuestra ventana “Environment” aparecio este nuevo vector
...
indica que es un dato de tipo *texto*. Pero en los ultimos años se
prefiere a utilizar la unidad *fct* (factor), de uso especial para el
trato de variables categoricas. La diferencia es bien marcada, ya que
los datos tipo texto, son eso, datos que nos sirven para evaluar texto
(insvestigaciones cualitativas, analisis de sentimiento, etc.). Sin
embargo, las variables tipo *fct* son estrictamente variables
categoricas que nos permiten correr analisis estadisticos sin errores.
...
Este tipo de vector, tiene variables del tipo *logi* (logicos), los
cuales nos permiten realizar operaciones con operadores boleano de
logica (visto en clase teorica).
...
palabra, se estila utizar guion bajo para unirlas, ya que el lenguaje R
considera a los espacios en blanco como errores si es que no son dentro
de una función. Los estilos para crear una variable compuesta pueden ser
los siguientes:
...
Vemos sutiles diferencias, trabajar con la forma que le paresca mas
...
Las operaciones tanto aritmeticas, como logicas se pueden realizar en
...
tibbles. En algunos casos utilizaremos tambien vectores, por lo cual es
importante manejarlos correctamente.

## 4. Manejo de bases de datos básico, acceder a una variable específica

Mas adelante veremos el paquete “dplyr” que nos permitira modificar y
manipular bases de datos, hay sin embargo dos maneras “antiguas” pero
aun utiles al momento de ingresar a una variable de una base de datos.

Podemos ingresar utilizando el simbolo de dolar **“$”** o utilizando
dobles corchetes ***\[\[\]\]***. Tenemos la base de datos “mpg” cargada.
Podemos observar al detalle que hay adentro con la funcion View():

``` r
View(mpg)
```

Este comando nos abrira un formato ordenado de la base de datos, donde
...
antes vamos a asignar el resultado de la eleccion de una variable a una
...
los corchetes colocaremos la posicion de la variable que queramos
seleccionar, en este caso la variabel año (“year”) se encuentra en la
...
Llamamos a la variable con combre “Seleccion\_variable2”:
...
curso de estadistica aplicada o los veamos sutilmente en los ultimos
temas. Con lo aprendido en esta primera parte introductoria basta para
comenzar a aprender especificamente que hacen diferentes paquetes y para
...
En el siguiente link, ustedes podran encontrar todas la “hojas de
...
Nota: Algunas de las guias se encuentran traducidas a diferentes idiomas
...
5.  Obténga la raíz cuadrada de los números redondeados con dos cifras
    decimales.
...
anteriormente y devuelve un vector logico. Utilicelas en la base de
datos mpg en alguna variable de su interes.
